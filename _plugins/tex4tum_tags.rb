  ...
  class SvgImage < Liquid::Tag
    ...
    def render(context)
      ...
      site.static_files.each do |file|
        return "<img class='img-fluid w-100' src='#{file.relative_path}'>" if @filename.include?(file.name)
      end
    end
  end
  ...
  class TabBox < Liquid::Block
    ...
    def render(context)
      ...
      text.scan(/((?<!#)\#{#{level}}\s+(.*))\n/) do |match| # /((?<!#)\#{#{level}}\s+(.*))\n/
        ...
        newprelabelhtml = newprelabelhtml.sub('"nav-link"', '"nav-link active"') if @@theid == startid
        ...
        newpretabhtml = newpretabhtml.sub('"tab-pane"', '"tab-pane active"') if @@theid == startid
