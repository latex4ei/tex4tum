...
A = ColumnDataSource(data=dict(x=[T.data['x'][1]/4,T.data['x'][1]/4], y=[0,1]))  # A
...
amp_slider = Slider(start=0.1, end=2.5, value=1, step=.1, title="Amplitude")
freq_slider = Slider(start=0.4, end=4, value=1, step=.1, title="Frequency")
phase_slider = Slider(start=-2, end=2, value=0, step=.1, title="Phase")
offset_slider = Slider(start=-1, end=1, value=0, step=.1, title="Offset")
...
callback = CustomJS(args=dict(source=source, Td=T, Ad=A, Pd=P, amp=amp_slider, freq=freq_slider, phase=phase_slider, offset=offset_slider),
                    code="""
